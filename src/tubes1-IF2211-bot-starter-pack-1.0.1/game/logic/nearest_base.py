...
class NearestBase(BaseLogic):
    ...
    def portal_utility_displacement(self, target:str, current_position: Position, portal_one_position: Position, portal_two_position: Position, board: Board, board_bot: GameObject):
        ...
        if target == "DiamondGameObject":
            ...
            for diamond in board.diamonds:
                displacement_to_diamond.append((diamond.position, diamond.properties.points/self.displacement(board_bot.properties.base,diamond.position), self.displacement(portal_to_finish_displacement,diamond.position)))
            shortest_displacement = max(displacement_to_diamond, key=lambda x: x[1])
            shortest_displacement_to_diamond = shortest_displacement[2]
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        if props.diamonds >= 4:
            ...
            listRatio = []
            for diamond in board.diamonds:
                print("Base position : ", board_bot.properties.base)
                print("Diamond position : ", diamond.position)
                print("Density from base : ", diamond.properties.points/self.displacement(board_bot.properties.base,diamond.position))
                if props.diamonds == 4:
                    if diamond.properties.points != 2:
                        listRatio.append((diamond.position, diamond.properties.points/self.displacement(board_bot.properties.base,diamond.position))) 
                else:
                    listRatio.append((diamond.position, diamond.properties.points/self.displacement(board_bot.properties.base,diamond.position)))
            try:
                ...
                if (self.displacement(current_position, maxDiamond[0]) > effective_portal_diamond_displacement):
                    self.goal_position = initial_portal_position
                else:
                    print('density triggered')
